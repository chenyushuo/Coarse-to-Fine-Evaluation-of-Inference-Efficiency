...
model = transformers.AutoModelForCausalLM.from_pretrained("/run/user/1000/Llama-2-7b-chat-hf", torch_dtype=torch.bfloat16,config=configuration).eval().cuda()
outputs = []
...
tokenizer = transformers.AutoTokenizer.from_pretrained("/run/user/1000/Llama-2-7b-chat-hf", padding_side="left")
...
with open(f"./data/tests_" + sys.argv[1] +"_" +sys.argv[2]+ ".pkl" , 'wb') as f:
